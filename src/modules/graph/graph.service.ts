...
export class GraphService {
  ...
  async getActualWorkAmountPerDay(
    ...
    // Generate series for days in the selected month (or current month if null, basically 1-31)
    // Adjust logic to correctly handle days of specific month if needed, but 1-31 covers max.
    // However, simplest is to just group by actual date if we want exact dates.
    // For a nice graph 1-31, we can use generate_series(1, 31).

    // NOTE: generate_series(1, 31) might produce invalid dates for Feb/Apr etc if we construct date.
    // But safely we can just check EXTRACT(DAY).

    const actualWorkAmountPerDay = await getManager().query(
      `WITH days AS (SELECT * FROM generate_series(1, 31) AS t(n))
      SELECT
        d.n::text AS mon, -- Reusing 'mon' alias to match frontend expectation or Resource
        COALESCE(SUM ( purchase_orders.actual_work_amount ), 0) AS sum,
        status
      FROM
      days d LEFT JOIN
        purchase_orders
        ON EXTRACT(DAY from purchase_orders.actual_work_date) = d.n 
        ${month ? `AND EXTRACT(MONTH from purchase_orders.actual_work_date) = ${month}` : ''}
        ${year ? `AND EXTRACT(YEAR from purchase_orders.actual_work_date) = ${year}` : ''}
      WHERE
        (purchase_orders.deleted_at IS NULL OR purchase_orders.deleted_at IS NULL) -- Logic to allow NULL join but filter POs
        AND (purchase_orders.id IS NULL OR (purchase_orders.deleted_at IS NULL ${whereQuery}))
      GROUP BY
        d.n, status
      ORDER BY d.n ASC`,
      whereParam,
    ...
    const filtered = [];
    actualWorkAmountPerDay.map((data) => {
      // Logic to merge status? Or just plain list?
      // The original perMonth does complex mapping with ActualWorkAmountPerMonthResource.
      // Let's reuse that structure if possible or simplify.
      const findData = filtered.find((f) => {
        return f.mon === data.mon;
      });
