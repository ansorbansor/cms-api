    ...
    raw.forEach(r => {
      if (r.customer_id) {
        ...
        if (!uniqueCustomers.has(cName)) {
          uniqueCustomers.set(cName, { id: r.customer_id, name: cName });
        }
      }
      if (r.project_id) {
        const pName = (r.project_name || '').trim();
        if (!uniqueProjects.has(pName)) {
          uniqueProjects.set(pName, { id: r.project_id, name: pName });
        }
      }
    });
